                ...
                <div class="grid grid-cols-2 mt-2 2xl:gap-5 gap-2">
                    @foreach (\App\Models\Menu::where('category_id', $item->id)->get() as $menu)
                        <div wire:click="showOrderModal({{ $menu->id }})"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-12 text-green-600" viewBox="0 0 24 24" fill="none"
                            ...
                            <div class="mt-2 text-sm ">
                                ...
                                <span class="text-gray-700 font-medium">&#8369;{{ number_format($menu->amount, 2) }}</span>
                            </div>
                        </div>
                    @endforeach
    ...
    <x-modal name="simpleModal" wire:model.defer="order_modal">
        <x-card title="{{ strtoupper($menuss->name ?? '') }}">
            <div class="w-[23rem]">
                <div class="bg-white hover:bg-gray-100  rounded-xl p-5">
                    ...
                    <div class="mt-2">
                        ...
                        <p class="text-xs">Remaining Stocks: {{$menuss->quantity ?? ''}}</p>
                    </div>
                </div>
                <div class="mt-3 px-5">
                    <x-number label="Quantity" wire:model.live="quantity" placeholder="0" />
                    @if(($menuss->quantity ?? 0) < ($quantity ?? 0))
                        <p class="text-xs text-red-500">Stocks should be greater than the quantity</p>
                    @endif
                </div>
            ...
            <x-slot name="footer" class="flex justify-end gap-x-4">
                ...
                <x-button positive :disabled="($menuss->quantity ?? 0) < ($quantity ?? 0)" right-icon="shopping-cart"
                    class="font-medium" label="Add to cart" wire:click="addToCart" spinner="addToCart" />
            </x-slot>
        </x-card>
    </x-modal>
</div>